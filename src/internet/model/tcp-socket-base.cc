...
void
TcpSocketBase::SetFlowId (uint32_t fd)
{
  m_flowId = fd;
}

void
TcpSocketBase::SetOutputFileName (std::string input)
{
...
TcpSocketBase::GenerateOutPut()
{
  ...
  *os << "[:" << m_node->GetId() << ":][+" << m_flowId << "+][#" << m_goodput / 1000000 << "#][<" << m_nextTxSequence.Get().GetValue() - 2<< ">][*"
      << (Simulator::Now().GetSeconds() - m_FSTime) << "*][$" << timeOuts << "$][!" << fReTx << "!][(" << pAckHit << ")][@" << fAck << "@][^"
      << fRecovery << "^][%" << tid.GetName() << "%][?" << m_rtt->GetCurrentEstimate().GetSeconds() << "?]" << std::endl;
}
