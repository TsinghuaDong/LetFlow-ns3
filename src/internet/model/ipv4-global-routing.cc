...
{ 
  static TypeId tid = TypeId ("ns3::Ipv4GlobalRouting")
    ...
    .AddAttribute ("RandomEcmpRouting",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("LetFlowRouting",
                   "Set to true if LetFlow is used",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_letFlowRouting),
                   MakeBooleanChecker ())
...
//(Ipv4Address dest, Ptr<NetDevice> oif)
Ptr<Ipv4Route>
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      else if ((allRoutes.size () > 1) && m_letFlowRouting && letFlow)
        {
          NS_LOG_INFO ("Letflow enabled! " << node->GetId());
          selectIndex = letFlow->ChooseRoute(allRoutes.size (), header, ipPayload);
        }
      else 
        {
          selectIndex = 0;
